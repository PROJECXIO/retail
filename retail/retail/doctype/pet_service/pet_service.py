	...
	def check_discount_values(self):
		...
		for item in self.service_items:
			if item.discount_as == "Percent" and flt(item.discount) > 100:
				frappe.throw(_("Discount Percent at row {} can not be greater that 100").format(item.idx))
			if item.discount_as == "Fixed Amount" and flt(item.discount) >  flt(item.rate):
				frappe.throw(_("Discount Amount at row {} can not be greater that rate {}").format(item.idx, item.rate))
			
			total_price += flt(item.rate)
			if item.discount_as == "Percent":
				total_net_price += (flt(item.rate) - (flt(item.rate) * flt(item.discount) / 100))
