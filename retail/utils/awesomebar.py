...
def _subseq_stats(text: str, pat: str) -> Tuple[int, int]:
    if not pat: return 0, 10**9
    t, p = text.lower(), pat.lower()
    ti = 0; first = -1; last = -1; matched = 0
    for ch in t:
...
def _score_subseq(q_raw: str, cand_text: str, tokens: Iterable[str]) -> float:
    ...
    if toks:
        ...
        if any(text.startswith(t) for t in toks): base += 0.05
...
def _trigram_sim(a: str, b: str) -> float:
    A = set(_trigrams(a)); B = set(_trigrams(b))
...
def _get_text_fields_row(r: dict, fold: bool) -> tuple[str, str]:
    ...
    if fold:
        code_f = _fold(code); name_f = _fold(name); agg_f = _fold(agg)
...
def custom_search_subsequence(
    ...
    *,
    ...
    """
    try:
        lim = int(limit);  lim = lim if lim > 0 else 200
    except Exception:
        lim = 200
    ...
    tokens = [_fold(t) for t in tokens_raw] if do_fold else [t.lower() for t in tokens_raw]
    ...
    if mode == "prefix":
        for r in rows:
            ...
            score = 0.0
            if any(code_f.startswith(t) for t in tokens): score += 1.0
            if any((r.get("item_name") or "").lower().startswith(t) for t in tokens): score += 0.6
            if any(agg_f.startswith(t) for t in tokens): score += 0.4
            ...
            if tri_terms:
                ...
                for r in ft_rows:
                    if r["item_code"] not in codes_seen:
                        ...
                        s_text = _fold(r["search_content"]) if do_fold else r["search_content"].lower()
