...
erpnext.PointOfSale.Payment = class {
	...
	bind_events() {
		...
		this.$component.on("click", ".submit-order-btn", () => {
			...
			const { custom_allow_partial_pos_payment, customer_name } = this.events.get_customer_details();
			this.allow_partial_payment = custom_allow_partial_pos_payment;
			if (
				!items.length ||
				(paid_amount == 0 &&
				...
				const message = items.length
					? __("You cannot submit the order without payment for customer {}.", [`<b>${customer_name}</b>`])
