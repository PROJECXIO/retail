...
def get_sales_invoice_item_from_consolidated_invoice(
    return_against_pos_invoice, pos_invoice_item
):
    try:
...
class POSInvoice(BasePOSInvoice):
    ...
    def validate(self):
        if not cint(self.is_pos):
            frappe.throw(
                _("POS Invoice should have the field {0} checked.").format(
                    frappe.bold(_("Include Payment"))
                )
            )
    ...
    def before_cancel(self):
        if (
            ...
            and frappe.db.get_value(
                "Sales Invoice", self.consolidated_invoice, "docstatus"
            )
            == 1
        ):
            ...
            frappe.throw(
                ...
                ).format(get_link_to_form("POS Closing Entry", pos_closing_entry[0])),
                title=_("Not Allowed"),
            )
    ...
    def clear_unallocated_mode_of_payments(self):
        ...
        frappe.qb.from_(sip).delete().where(sip.parent == self.name).where(
            sip.amount == 0
        ).run()
    ...
    def create_return_sales_invoice(self):
        ...
        map_doc(
            ...
            table_map={"doctype": return_sales_invoice.doctype},
        )
        ...
        for d in self.items:
            si_item = map_child_doc(
                d, return_sales_invoice, {"doctype": "Sales Invoice Item"}
            )
        ...
        for d in self.get("taxes"):
            tax = map_child_doc(
                d, return_sales_invoice, {"doctype": "Sales Taxes and Charges"}
            )
            taxes.append(tax)

        for d in self.get("payments"):
            payment = map_child_doc(
                d, return_sales_invoice, {"doctype": "Sales Invoice Payment"}
            )
        ...
        for pay in self.payments:
            if pay.type == "Phone" and pay.amount >= 0:
                ...
                if paid_amt and pay.amount != paid_amt:
                    return frappe.throw(
                        _("Payment related to {0} is not completed").format(
                            pay.mode_of_payment
                        )
                    )
    ...
    def validate_stock_availablility(self):
        ...
        for d in self.get("items"):
            if not d.serial_and_batch_bundle:
                ...
                available_stock, is_stock_item, delivered_by_supplier = (
                    get_stock_availability(d.item_code, d.warehouse, self.pos_profile)
                )
                ...
                if (
                    is_stock_item
                    and flt(available_stock) <= 0
                    and not delivered_by_supplier
                ):
                    frappe.throw(
                        _(
                    ...
                    is_stock_item
                    and flt(available_stock) < flt(d.stock_qty)
                    and not delivered_by_supplier
                ):
                    frappe.throw(
                        _(
    ...
    def validate_serialised_or_batched_item(self):
        ...
        if error_msg:
            frappe.throw(
                error_msg, title=_("Serial / Batch Bundle Missing"), as_list=True
            )
    ...
    def validate_mode_of_payment(self):
        if len(self.payments) == 0 and sel.status != "Unpaid":
    ...
    def validate_change_account(self):
        if (
            ...
            and frappe.get_cached_value(
                "Account", self.account_for_change_amount, "company"
            )
            != self.company
        ):
            frappe.throw(
                ...
                ).format(self.account_for_change_amount, self.company)
            )
    ...
    def validate_change_amount(self):
        ...
        if not flt(self.change_amount) and grand_total < flt(self.paid_amount):
            self.change_amount = flt(
                self.paid_amount - grand_total + flt(self.write_off_amount)
            )
            self.base_change_amount = (
                flt(self.base_paid_amount)
                - base_grand_total
        ...
        if flt(self.change_amount) and not self.account_for_change_amount:
            frappe.msgprint(
                _("Please enter Account for Change Amount"), raise_exception=1
            )
    ...
    def validate_payment_amount(self):
        ...
        if self.is_return and self.docstatus != 0:
            ...
            total_amount_in_payments = flt(
                total_amount_in_payments, self.precision("grand_total")
            )
    ...
    def set_outstanding_amount(self):
        ...
        self.outstanding_amount = (
            total - flt(self.paid_amount) if total > flt(self.paid_amount) else 0
        )

    def validate_loyalty_transaction(self):
        if self.redeem_loyalty_points and (
            not self.loyalty_redemption_account
    ...
    def set_status(self, update=False, status=None, update_modified=True):
        ...
        if not status:
            if self.docstatus == 2:
                ...
                elif flt(self.outstanding_amount) > 0 and getdate(
                    self.due_date
                ) < getdate(nowdate()):
                    self.status = "Overdue"
                elif (
    ...
    def set_pos_fields(self, for_validate=False):
        ...
        if profile:
            ...
            self.account_for_change_amount = (
                profile.get("account_for_change_amount")
                or self.account_for_change_amount
            )
            ...
            if self.customer:
                customer_price_list, customer_group, customer_currency = (
                    frappe.db.get_value(
                        "Customer",
                        self.customer,
    ...
    def update_payments(self, payments):
        ...
        if paid_amount >= total:
            frappe.throw(
                title=_("Invoice Paid"), msg=_("This invoice has already been paid.")
            )
        ...
        base_paid_amount = flt(
            flt(paid_amount * self.conversion_rate), self.precision("base_paid_amount")
        )
        outstanding_amount = (
            flt(flt(total - paid_amount), self.precision("outstanding_amount"))
            if total > paid_amount
            else 0
        )
        change_amount = (
            flt(flt(paid_amount - total), self.precision("change_amount"))
            if paid_amount > total
            else 0
        )
...
def get_stock_availability(item_code, warehouse, pos_profile):
    ...
    delivered_by_supplier = frappe.db.get_value(
        "Item", item_code, "delivered_by_supplier"
    )
...
def get_pos_reserved_qty(item_code, warehouse):
    ...
    pinv_item_reserved_qty = get_pos_reserved_qty_from_table(
        "POS Invoice Item", item_code, warehouse
    )
    packed_item_reserved_qty = get_pos_reserved_qty_from_table(
...
def create_payments_on_invoice(doc, idx, payment_details):
    ...
    payment.account = get_bank_cash_account(payment.mode_of_payment, doc.company).get(
        "account"
    )
...
def get_invoice_item_returned_qty(doctype, invoice, customer, item_row_name):
    is_return, docstatus = frappe.db.get_value(
        doctype, invoice, ["is_return", "docstatus"]
    )
...
def is_invoice_returnable(doctype, invoice):
    ...
    invoice_item_qty = frappe.db.get_all(
        f"{doctype} Item", {"parent": invoice}, ["name", "qty"]
    )
...
def get_returned_qty_map_for_row(return_against, party, row_name, doctype):
    ...
    reference_field = (
        "dn_detail" if doctype == "Delivery Note" else frappe.scrub(child_doctype)
    )
    ...
    if doctype in ("Purchase Receipt", "Purchase Invoice", "Subcontracting Receipt"):
        ...
        if doctype == "Purchase Receipt":
            fields += [
                f"sum(abs(`tab{child_doctype}`.received_stock_qty)) as received_stock_qty"
            ]
