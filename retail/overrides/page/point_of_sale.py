...
def search_by_term(search_term, warehouse, price_list, pos_profile):
    ...
    item_stock_qty, is_stock_item, delivered_by_supplier = get_stock_availability(
        item_code, warehouse, pos_profile
    )
...
def filter_result_items(result, pos_profile):
    if result and result.get("items"):
        ...
        result["items"] = [
            ...
            if item.get("item_group") in pos_item_groups
        ]
...
def get_items(start, page_length, price_list, item_group, pos_profile, search_term=""):
    ...
    for item in items_data:
        item.actual_qty, _, delivered_by_supplier = get_stock_availability(
...
def get_past_order_list(search_term, status, limit=20):
    ...
    invoice_list = order_results_by_posting_date(
        [*pos_invoice_list, *sales_invoice_list]
    )
