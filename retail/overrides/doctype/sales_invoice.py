...
class SalesInvoice(SellingController):
			...
			if cost_center_company != self.company:
				frappe.throw(
					_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
						frappe.bold(item.idx), frappe.bold(self.cost_center), frappe.bold(self.company)
					)
				)
	...
	def validate_full_payment(self):
		...
		allow_partial_payment = frappe.db.get_value("Customer", self.customer, "custom_allow_partial_pos_payment")
