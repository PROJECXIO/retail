...
from erpnext.accounts.doctype.pos_closing_entry.pos_closing_entry import POSClosingEntry as BasePOSClosingEntry
...
class POSClosingEntry(BasePOSClosingEntry):
	...
	def update_sales_invoices_closing_entry(self, cancel=False):
		for d in self.custom_sales_invoice_reference:
			frappe.db.set_value(
				"Sales Invoice", d.sales_invoice, "custom_pos_closing_entry", self.name if not cancel else None
			)
