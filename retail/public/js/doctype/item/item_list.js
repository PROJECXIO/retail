(function () {
  ...
  function mount_in_page_form(listview, $el) {
    ...
    if (!$form || !$form.length) {
      (listview.$page || $(listview.page.wrapper)).find(".page-head").append($el);
    } else {
      $form.append($el);
    }
  }
  ...
  function get_other_filters(listview) {
    try {
      ...
      const curr = fa.get() || [];
      return curr.filter(f => !(Array.isArray(f) && f[0] === DOCTYPE && f[1] === "item_code"));
    } catch (e) {
      return [];
    }
  }
  ...
  function reset_filters_with_item_code(listview, otherFilters, itemCodes, useNotSet = false) {
    const fa = listview.filter_area;
    try {
      if (fa && typeof fa.clear === "function") fa.clear();
      else {
        const curr = (fa && fa.get && fa.get()) || [];
        curr.forEach(f => { try { fa.remove([f]); } catch(e){} });
      }
  ...
  // -------------------------
  // Attach to Item ListView
  // -------------------------
  frappe.listview_settings[DOCTYPE] = $.extend({}, frappe.listview_settings[DOCTYPE] || {}, {
    onload(listview) {
      ...
      const debouncedApply = debounce(function () {
        const val = ($input.val() || "");
        try { localStorage.setItem(STORAGE_KEY, val.trim()); } catch(e){}
      ...
      $input.on("keydown", function (e) {
        if (e.key === "Enter") {
          e.preventDefault();
          try { localStorage.setItem(STORAGE_KEY, ($input.val() || "").trim()); } catch(e){}
