...
from erpnext.controllers.selling_controller import (
    SellingController,
...
def reverse_depreciation_entry_made_on_disposal(asset):
    for row in asset.get("finance_books"):
        schedule_doc = get_asset_depr_schedule_doc(
        ...
        for schedule_idx, schedule in enumerate(
            schedule_doc.get("depreciation_schedule")
        ):
            if schedule.schedule_date == asset.disposal_date and schedule.journal_entry:
                if not disposal_was_made_on_original_schedule_date(
                    ...
                    je = create_reverse_depreciation_entry(
                        asset.name, schedule.journal_entry
                    )
...
def normalize_ctx_input(T: type) -> callable:
    ...
    def decorator(func: callable):
        ...
        wrapper.__annotations__.update(
            {k: v for k, v in func.__annotations__.items() if k != "ctx"}
        )
        return wrapper
...
@normalize_ctx_input(ItemDetailsCtx)
...
class SalesInvoice(SellingController):
    ...
    def split_quantity(self):
        ...
        for item in self.items:
            ...
            if flt(item.qty) > actual_qty:
                if self.docstatus == 1:
                    item.qty = actual_qty
                shows_warehouses = frappe.get_all(
                    "Item Supplier",
                    filters={
                        "parenttype": "Item",
            ...
            self.append(
                "custom_pos_drop_ship_item_details",
                {
                    "item_code": item.item_code,
    ...
    def validate(self):
        ...
        is_deferred_invoice = any(
            d.get("enable_deferred_revenue") for d in self.get("items")
        )
                ...
                if d.asset:
                    if not self.is_return:
                        ...
                        if self.update_stock:
                            ...
                            frappe.throw(
                                _(
                                    "Row #{0}: Asset {1} cannot be sold, it is already {2}"
                                ).format(d.idx, d.asset, asset_status)
                            )
                        elif asset_status == "Sold" and not self.is_return:
                            frappe.throw(
                                _("Row #{0}: Asset {1} is already sold").format(
                                    d.idx, d.asset
                                )
                            )
                        ...
                        frappe.throw(
                            _(
                                "Row #{0}: Return Against is required for returning asset"
                            ).format(d.idx)
                        )
                else:
                    frappe.throw(
    ...
    def validate_income_account(self):
        for item in self.get("items"):
            validate_account_head(
                item.idx, item.income_account, self.company, _("Income")
            )
    ...
    def handle_used_from_show_qty(self):
        ...
        for item in self.items:
            ...
            if (
                flt(item_supplier.custom_quantity_supplied_for_shows) == 0
                or not item_supplier.custom_supplier_warehouse
            ):
                continue
            ...
            items_for_each_supplier.update(
                {
                    f"{item_supplier.supplier}": {
                        ...
                        "item_supplier": {
                            ...
                            "qty": flt(
                                item_supplier.custom_quantity_supplied_for_shows
                            ),
                        },
                    },
                }
        ...
        for i, item in enumerate(self.items):
            ...
            if cint(item.delivered_by_supplier) == 1:
                ...
                if not default_supplier or default_supplier is None:
                    frappe.throw(
                        _(
                            "Item at Row #{} is delivered by supplier set default supplier for item {}"
    ...
    def check_if_created_using_pos_and_pos_closing_entry_generated(self):
        if (
            ...
            and self.custom_pos_closing_entry
        ):
            ...
            if pos_closing_entry_docstatus == 1:
                frappe.throw(
                    msg=_(
                        "To cancel this Sales Invoice you need to cancel the POS Closing Entry {}."
                    ).format(
                        get_link_to_form(
                            "POS Closing Entry", self.custom_pos_closing_entry
                        )
                    ),
                    title=_("Not Allowed"),
                )
    ...
    def on_cancel(self):
        ...
        unlink_inter_company_doc(
            self.doctype, self.name, self.inter_company_invoice_reference
        )
    ...
    def set_account_for_mode_of_payment(self):
        for payment in self.payments:
            payment.account = get_bank_cash_account(
                payment.mode_of_payment, self.company
            ).get("account")
    ...
    def get_company_abbr(self):
        return frappe.db.sql("select abbr from tabCompany where name=%s", self.company)[
            0
        ][0]
    ...
    def clear_unallocated_mode_of_payments(self):
        self.set(
            "payments", self.get("payments", {"amount": ["not in", [0, None, ""]]})
        )
    ...
    def validate_created_using_pos(self):
        if self.is_created_using_pos and not self.pos_profile:
            frappe.throw(
                _("POS Profile is mandatory to mark this invoice as POS Transaction.")
            )

        # self.invoice_type_in_pos = frappe.db.get_single_value("POS Settings", "invoice_type")
        self.invoice_type_in_pos = "Sales Invoice"
    ...
    def validate_full_payment(self):
        ...
        allow_partial_payment = frappe.db.get_value(
            "Customer", self.customer, "custom_allow_partial_pos_payment"
        )
    ...
    def validate_pos_opening_entry(self):
        ...
        if (
            frappe.utils.get_date_str(opening_entries[0].get("period_start_date"))
            != frappe.utils.today()
        ):
            frappe.throw(
    ...
    def validate_warehouse(self):
        ...
        for d in self.get_item_list():
            ...
            if (
                ...
                frappe.throw(
                    _("Warehouse required for stock Item {0}").format(d.item_code)
                )
    ...
    def validate_delivery_note(self):
        ...
        if notes:
            frappe.throw(
                _(
                    "Stock cannot be updated against the following Delivery Notes: {0}"
                ).format(comma_and(notes)),
            )
    ...
    def is_auto_fetch_timesheet_enabled(self):
        return frappe.db.get_single_value(
            "Projects Settings", "fetch_timesheet_in_sales_invoice"
        )
    ...
    def get_warehouse(self):
        ...
        if not warehouse:
            ...
            if global_pos_profile:
                ...
                msgprint(
                    _("POS Profile required to make POS Entry"), raise_exception=True
                )
    ...
    def set_income_account_for_fixed_assets(self):
        ...
        for item in self.items:
            ...
            item.income_account = disposal_account
            if not item.cost_center:
                item.cost_center = depreciation_cost_center
        ...
        for d in self.get("items"):
            if (
                ...
                and frappe.db.get_value(
                    "Sales Order", d.sales_order, "docstatus", cache=True
                )
                != 1
            ):
                frappe.throw(
                    _("Sales Order {0} is not submitted").format(d.sales_order)
                )

            if (
                d.delivery_note
                and frappe.db.get_value(
                    "Delivery Note", d.delivery_note, "docstatus", cache=True
                )
                != 1
            ):
    ...
    def process_asset_depreciation(self):
        if (self.is_return and self.docstatus == 2) or (
            not self.is_return and self.docstatus == 1
        ):
        ...
        for d in self.get("items"):
            if d.asset:
                ...
                if asset.calculate_depreciation and asset.status != "Fully Depreciated":
                    depreciate_asset(
                        asset, disposal_date, self.get_note_for_asset_sale(asset)
                    )

    def get_note_for_asset_sale(self, asset):
        return _(
            "This schedule was created when Asset {0} was {1} through Sales Invoice {2}."
        ).format(
    ...
    def update_asset(self):
        ...
        for d in self.get("items"):
            if d.asset:
                ...
                if (self.is_return and self.docstatus == 1) or (
                    not self.is_return and self.docstatus == 2
                ):
                    ...
                    note = (
                        _("Asset sold")
                        if not self.is_return
                        else _("Return invoice of asset cancelled")
                    )
                    asset_status = "Sold"
    ...
    def get_disposal_date(self):
        if self.is_return:
            disposal_date = frappe.db.get_value(
    ...
    def make_gl_entries(self, gl_entries=None, from_repost=False):
        ...
        if gl_entries:
            ...
            update_outstanding = (
                ...
                if (
                    cint(self.is_pos)
                    or self.write_off_account
                    or cint(self.redeem_loyalty_points)
                )
                else "Yes"
            )
    ...
    def make_customer_gl_entry(self, gl_entries):
        ...
        if grand_total and not self.is_internal_transfer():
            ...
            if (
                self.is_return
                and self.return_against
                and not self.update_outstanding_for_self
            ):
    ...
    def make_item_gl_entries(self, gl_entries):
                        ...
                        self.get_gl_dict(
                            {
                                ...
                                "against": self.customer,
                                "credit": flt(
                                    base_amount, item.precision("base_net_amount")
                                ),
                                "credit_in_account_currency": (
                                ...
                                "credit_in_transaction_currency": flt(
                                    amount, item.precision("net_amount")
                                ),
        ...
        if cint(self.update_stock) and erpnext.is_perpetual_inventory_enabled(
            self.company
        ):
        ...
        if cint(self.is_pos):
            ...
            for payment_mode in self.payments:
                ...
                if payment_mode.base_amount:
                        ...
                        self.get_gl_dict(
                            {
                                ...
                                "credit_in_account_currency": (
                                    ...
                                    if self.party_account_currency
                    ...
                    payment_mode_account_currency = get_account_currency(
    ...
    def get_gle_for_change_amount(self) -> list[dict]:
        ...
        if not self.account_for_change_amount:
            frappe.throw(
                _("Please set Account for Change Amount"), title=_("Mandatory Field")
            )
                ...
                self.get_gl_dict(
                    {
                        ...
                        "credit_in_account_currency": (
                            ...
                            if self.party_account_currency == self.company_currency
                                ...
                                self.precision("write_off_amount"),
                            )
                        ),
                        "credit_in_transaction_currency": flt(
    ...
    def verify_payment_amount_is_positive(self):
        for entry in self.payments:
            if entry.amount < 0:
                frappe.throw(
                    _("Row #{0} (Payment Table): Amount must be positive").format(
                        entry.idx
                    )
                )
    ...
    def verify_payment_amount_is_negative(self):
        for entry in self.payments:
            if entry.amount > 0:
                frappe.throw(
                    _("Row #{0} (Payment Table): Amount must be negative").format(
                        entry.idx
                    )
                )
    ...
    def delete_loyalty_point_entry(self):
        lp_entry = frappe.db.sql(
            "select name from `tabLoyalty Point Entry` where invoice=%s",
            (self.name),
            as_dict=1,
        )
        ...
        if against_lp_entry:
            ...
            frappe.db.sql(
                """delete from `tabLoyalty Point Entry` where invoice=%s""", (self.name)
            )
            # Set loyalty program
            self.set_loyalty_program_tier()
    ...
    def apply_loyalty_points(self):
        ...
        for lp_entry in loyalty_point_entries:
            if lp_entry.invoice_type != self.doctype or lp_entry.invoice == self.name:
                ...
                continue
            available_points = lp_entry.loyalty_points - flt(
                redemption_details.get(lp_entry.name)
            )
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
    ...
    if doctype in ["Sales Invoice", "Sales Order"]:
        partytype, ref_partytype, internal = (
            "Customer",
            "Supplier",
    ...
    if inter_company_reference:
        ...
        if (
            frappe.db.get_value(partytype, {"represents_company": doc.company}, "name")
            != party
        ):
            frappe.throw(
                _("Invalid {0} for Inter Company Transaction.").format(_(partytype))
            )
        if (
            frappe.get_cached_value(ref_partytype, ref_party, "represents_company")
            != company
        ):
...
def get_inter_company_details(doc, doctype):
    if doctype in ["Sales Invoice", "Sales Order", "Delivery Note"]:
        ...
        company = frappe.get_cached_value(
            "Supplier", doc.supplier, "represents_company"
        )
...
def validate_inter_company_transaction(doc, doctype):
    ...
    if not valid_price_list and not doc.is_internal_transfer():
        frappe.throw(
            _("Selected Price List should have buying and selling fields checked.")
        )

    party = details.get("party")
    if not party:
        partytype = (
            "Supplier" if doctype in ["Sales Invoice", "Sales Order"] else "Customer"
        )
        frappe.throw(
            _("No {0} found for Inter Company Transactions.").format(partytype)
        )
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
    if doctype in ["Sales Invoice", "Sales Order"]:
        ...
        target_doctype = (
            "Purchase Invoice" if doctype == "Sales Invoice" else "Purchase Order"
        )
        target_detail_field = (
        ...
        received_items = get_received_items(
            source_name, target_doctype, target_detail_field
        )
    else:
        source_doc = frappe.get_doc(doctype, source_name)
        target_doctype = (
            "Sales Invoice" if doctype == "Purchase Invoice" else "Sales Order"
        )
    ...
    def update_details(source_doc, target_doc, source_parent):
        ...
        if target_doc.doctype in ["Purchase Invoice", "Purchase Order"]:
            currency = frappe.db.get_value(
                "Supplier", details.get("party"), "default_currency"
            )
            ...
            currency = frappe.db.get_value(
                "Customer", details.get("party"), "default_currency"
            )
            target_doc.company = details.get("company")
    ...
    def update_item(source, target, source_parent):
        ...
        if (
            source.doctype == "Purchase Order Item"
            and target.doctype == "Sales Order Item"
        ):
    ...
    if doc.is_internal_transfer():
        if doc.doctype == "Purchase Receipt":
            ...
            if so_item_map:
                ...
                update_pr_items(
                    doc, so_item_map, pd_item_map, parent_child_map, warehouse_map
                )
...
def update_pi_items(
    ...
    for item in doc.get("items"):
        item.set(
            detail_field,
            purchase_item_map.get(sales_item_map.get(item.sales_invoice_item)),
        )
        item.set(
            parent_field,
            parent_child_map.get(sales_item_map.get(item.sales_invoice_item)),
        )
        if doc.update_stock:
            item.warehouse = warehouse_map.get(
                sales_item_map.get(item.sales_invoice_item)
            )
            if (
                not item.warehouse
...
def update_pr_items(
    doc, sales_item_map, purchase_item_map, parent_child_map, warehouse_map
):
    for item in doc.get("items"):
    ...
    for row in pos_profile.get("payments"):
        ...
        if not payment_mode:
            invalid_modes.append(
                get_link_to_form("Mode of Payment", row.mode_of_payment)
            )
            continue
    ...
    return get_mapped_doc(
        ...
        table_maps={
            ...
            "Payment Schedule": {
                ...
                and getdate(doc.due_date) < getdate(),
            },
        },
    ...
    if payment_entries:
        ...
        if links_to_pe:
            payment_entries_link = [
                get_link_to_form("Payment Entry", name, label=name)
                for name in links_to_pe
            ]
