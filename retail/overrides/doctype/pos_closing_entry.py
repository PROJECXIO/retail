...
from erpnext.accounts.doctype.pos_closing_entry.pos_closing_entry import (
    POSClosingEntry as BasePOSClosingEntry,
)

# from erpnext.controllers.status_updater import StatusUpdater


class POSClosingEntry(BasePOSClosingEntry):
    def validate(self):
    ...
    def validate_pos_opening_entry(self):
        if (
            frappe.db.get_value("POS Opening Entry", self.pos_opening_entry, "status")
            != "Open"
        ):
            frappe.throw(
                _("Selected POS Opening Entry should be open."),
    ...
    def validate_invoice_mode(self):
        ...
        if self.invoice_type == "Sales Invoice":
            if len(self.pos_transactions) != 0:
                frappe.throw(
                    _("POS Invoices can't be added when Sales Invoice is enabled")
                )
    ...
    def validate_duplicate_pos_invoices(self):
        ...
        for key, value in pos_occurences.items():
            if len(value) > 1:
                error_list.append(
                    _("{0} is added multiple times on rows: {1}").format(
                        frappe.bold(key), frappe.bold(value)
                    )
                )

        if error_list:
            frappe.throw(
                error_list, title=_("Duplicate POS Invoices found"), as_list=True
            )
    ...
    def validate_duplicate_sales_invoices(self):
        ...
        for idx, inv in enumerate(self.custom_sales_invoice_reference, 1):
        ...
        if error_list:
            frappe.throw(
                error_list, title=_("Duplicate Sales Invoices found"), as_list=True
            )

    def validate_sales_invoices(self):
        invalid_rows = []
        for d in self.custom_sales_invoice_reference:
            invalid_row = {"idx": d.idx}
            sales_invoice = frappe.db.get_values(
                ...
                [
                    ...
                    "is_consolidated",
                    "custom_pos_closing_entry",
                ],
                as_dict=1,
            )[0]
            if sales_invoice.custom_pos_closing_entry:
                invalid_row.setdefault("msg", []).append(
                    _("Sales Invoice is already consolidated")
                )
                invalid_rows.append(invalid_row)
                continue
            if sales_invoice.is_pos == 0:
                invalid_row.setdefault("msg", []).append(
                    _("Sales Invoice does not have Payments")
                )
            if sales_invoice.is_created_using_pos == 0:
                invalid_row.setdefault("msg", []).append(
                    _("Sales Invoice is not created using POS")
                )
            ...
            if sales_invoice.docstatus != 1:
                invalid_row.setdefault("msg", []).append(
                    _("Sales Invoice is not submitted")
                )
    ...
    def update_sales_invoices_closing_entry(self, cancel=False):
        for d in self.custom_sales_invoice_reference:
            frappe.db.set_value(
                ...
                d.sales_invoice,
                "custom_pos_closing_entry",
                self.name if not cancel else None,
            )

    def check_pce_is_cancellable(self):
        if frappe.db.exists(
            "POS Opening Entry", {"pos_profile": self.pos_profile, "status": "Open"}
        ):
            frappe.throw(
...
def get_invoices(start, end, pos_profile, user):
    ...
    sales_inv_query = build_invoice_query(
        "Sales Invoice", user, pos_profile, start, end
    )

    query = sales_inv_query

    if invoice_doctype == "POS Invoice":
        pos_inv_query = build_invoice_query(
            "POS Invoice", user, pos_profile, start, end
        )
        query = query + pos_inv_query

    query = query.orderby(query.timestamp)
    invoices = query.run(as_dict=1)
    payments = get_payments(invoices, pos_profile)
    data = {"invoices": invoices, "payments": payments, "taxes": get_taxes(invoices)}

    return data


def get_pos_profile_payments(pos_profile):
    ...
    return (
        ...
        .on(ModeOfPaymentAccount.parent == POSPaymentMethod.mode_of_payment)
        .left_join(POSProfile)
        .on(POSProfile.name == POSPaymentMethod.parent)
        .where(
            (POSPaymentMethod.parenttype == "POS Profile")
            & (POSPaymentMethod.parent == pos_profile)
            & (POSProfile.company == ModeOfPaymentAccount.company)
        )
        .groupby(POSPaymentMethod.mode_of_payment, ModeOfPaymentAccount.default_account)
        .select(
            POSPaymentMethod.mode_of_payment,
            ModeOfPaymentAccount.default_account.as_("account"),
            ConstantColumn(0).as_("amount"),
        )
    ).run(as_dict=1)


def get_payments(invoices, pos_profile):
    if not len(invoices):
        return get_pos_profile_payments(pos_profile)
...
def get_taxes(invoices):
    ...
    query = (
        ...
        .select(
            ...
            fn.Sum(SalesInvoiceTaxesCharges.tax_amount_after_discount_amount).as_(
                "tax_amount"
            ),
        )
    )
...
def make_closing_entry_from_opening(opening_entry):
    ...
    for d in data.get("invoices"):
        ...
        closing_entry.custom_total_taxes_and_charges += flt(d.total_taxes_and_charges)

    closing_entry.set("pos_invoices", pos_invoices)
    closing_entry.set("custom_sales_invoice_reference", sales_invoices)
...
def build_invoice_query(invoice_doctype, user, pos_profile, start, end):
    ...
    query = (
        ...
        .select(
            ...
            fn.Timestamp(InvoiceDocType.posting_date, InvoiceDocType.posting_time).as_(
                "timestamp"
            ),
        ...
        .where(
            ...
            & (
                (
                    fn.Timestamp(
                        InvoiceDocType.posting_date, InvoiceDocType.posting_time
                    )
                    >= start
                )
                & (
    ...
    if invoice_doctype == "POS Invoice":
        ...
        query = query.where(
            ...
            & fn.IfNull(InvoiceDocType.custom_pos_closing_entry, "").eq("")
        )
