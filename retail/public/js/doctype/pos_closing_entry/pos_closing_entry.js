...
frappe.ui.form.on("POS Closing Entry", {
	onload: async function (frm) {
		...
		frm.set_query("user", function (doc) {
			return {
				query: "retail.overrides.doctype.pos_closing_entry.get_cashiers",
	...
	refresh: function (frm) {
		...
		frappe.ui.form.off("POS Closing Entry", "pos_opening_entry", undefined);
		frappe.ui.form.on(
			"POS Closing Entry",
			"pos_opening_entry",
			pos_opening_entry2,
		...
		frappe.ui.form.off("POS Closing Entry", "set_opening_amounts", undefined);
		frappe.ui.form.on(
			"POS Closing Entry",
			"set_opening_amounts",
			set_opening_amounts
		...
		frappe.ui.form.off("POS Closing Entry", "get_invoices", undefined);
		frappe.ui.form.on(
			"POS Closing Entry",
			"get_invoices",
			get_invoices
...
function pos_opening_entry2(frm) {
	if (
...
function set_opening_amounts(frm) {
	return frappe.db
...
function get_invoices(frm) {
	return frappe.call({
		method: "retail.overrides.doctype.pos_closing_entry.get_invoices",
		args: {
...
function set_transaction_form_data(data, frm) {
	data.forEach((d) => {
		...
		frm.doc.custom_total_taxes_and_charges += flt(d.total_taxes_and_charges);
	});
}
...
function add_to_transaction(d, frm) {
	const field =
		d.doctype === "POS Invoice"
			? "pos_transactions"
			: "custom_sales_invoice_reference";
	frm.add_child(field, {
...
function reset_values(frm) {
	...
	frm.set_value("custom_sales_invoice_reference", []);
...
function refresh_fields(frm) {
	...
	frm.refresh_field("custom_sales_invoice_reference");
