...
erpnext.selling.POSInvoiceController = class POSInvoiceController extends (
  ...
  onload(doc) {
    ...
    erpnext.accounts.dimensions.setup_dimension_filters(
      this.frm,
      this.frm.doctype
    );
        ...
        .then((r) => {
          if (!r.exc) {
            this.frm.set_default_payment =
              r.message.set_grand_total_to_default_mop;
          }
        });
    }
  }
  ...
  pos_profile(frm) {
    ...
    frappe.call({
      method:
        "retail.overrides.page.point_of_sale.get_pos_profile_data",
  ...
  async collect_outstanding_payment() {
    const total_amount =
      flt(this.frm.doc.rounded_total) | flt(this.frm.doc.grand_total);
    ...
    const table_fields = [
      ...
      },
      {
        ...
        onchange: function () {
          ...
          dialog.fields_dict.change_amount.set_value(
            change_amount < 0 ? change_amount * -1 : 0
          );
        },
      },
    ];
...
extend_cscript(
  cur_frm.cscript,
  new erpnext.selling.POSInvoiceController({ frm: cur_frm })
);
  ...
  set_loyalty_points: function (frm) {
    if (frm.redemption_conversion_factor) {
      ...
      if (frm.doc.grand_total && remaining_amount < loyalty_amount) {
        ...
        frappe.throw(
          __("You can only redeem max {0} points in this order.", [
            redeemable_points,
          ])
        );
      }
